...
#test chiralrs
        ...
        for i in range(mol.GetNumAtoms()):
            for j in range(mol.GetNumAtoms()):
                ...
                if i !=j :
                    ...
                    g.add_edge(i,j,extend_dis=np.concatenate((np.exp(-(ext_dis-np.linspace(0,8,20))**2/(0.5**2)),np.exp(-(ext_dis-np.linspace(0,4,20))**2/(0.5**2))),0))
                    
                else:
                    g.add_edge(i,j,extend_dis=[0]*40)
    ...
    def shortestpathtotbonds(self,m,i,j):
        ...
        #  print ('path atom pair',path_atom_list[i],'   ' ,path_atom_list[i+1])
       # print('total bonds are',path_tot_bonds)
    ...
    def envatom(self,mol,j,i):  #j radius i atom index 
    ...
    def envascf(self,mol4ascf,position):
       ...
       acsf4atom = acsf.create(mol4ascf, positions=[position],n_jobs=12)
       return acsf4atom[0]
